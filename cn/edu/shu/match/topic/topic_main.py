...
def insert_into_match(require_id, provide_id, match_degree, algorithm_type):
    ...
    #logging.warning("require_vector {}".format(require_vector))
    #mongo.set_collection(mongodb_json['provide'])
    #provide_vector = mongo.get_collection().find_one({"provide_id": provide_id})
    #logging.warning("provide_vector {}".format(provide_vector))
    if Tmatch_degree > gl.cos_match_threshold:
        logging.warning("value {}".format(TopicUtils.get_cos_value(require_vector['require_vector'], provide_vector['provide_vector'])))
        ...
        if 1 == len(id_range_list):
            for require_id in range(0, max_require_id[0][0] + 1):
                ...
                for provide_id, match_degree in provide_dict.items():
                    logging.warning("provide_id {} match_agree {}".format(provide_id,match_degree))
